...
const Cart = () => {
  ...
  return (
    <>
      <div className={`fixed right-0 top-0 w-[20vw] h-full p-5 bg-white mb-3 ${
        activeCart ? "translate-x-0" : "translate-x-full"} transition-all duration-500 z-50`}
      >
        <div className="flex justify-between items-center my-3">
          <span className="text-xl font-bold text-gray-800">MY ORDER</span>
          <IoMdClose onClick={()=>setActiveCart(false)}
          className="border-2 border-gray-600 text-gray-600 font-bold p-1 text-xl rounded-md hover:text-red-300 cursor-pointer"/>
        </div>
        {
        ...
        <div className="absolute bottom-0">
          ...
          <button onClick={()=>Navigate("/success")} className="bg-green-500 font-bold px-3 text-white py-2 rounded-lg w-[17vw] mb-5">Checkout</button>
        </div>
